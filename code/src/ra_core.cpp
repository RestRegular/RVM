...
namespace core::id {
    TypeID::TypeID(std::string type_name, const base::IDType &idType, const std::string &dtype_ident)
            : RVM_ID('T', idType), type_name(std::move(type_name)), dtype_ident(dtype_ident) {}

    TypeID::TypeID(std::string type_name, const std::shared_ptr<TypeID> &parent_type,
                   const base::IDType &idType, const std::string &dtype_ident)
            : RVM_ID('T', idType), type_name(std::move(type_name)),
            dtype_ident(dtype_ident), parent_type_id(parent_type) {}
    ...
    std::string TypeID::getIdentStr() const {
        return dtype_ident;
    }
...
core::data::ExtensionType core::data::ExtensionTypeTools::getExtensionType(const std::string &extension_path) {
    if (extension_path.ends_with(".ra")) {
        return core::data::ExtensionType::RA;
    } else if (extension_path.ends_with(".rsi")) {
        return core::data::ExtensionType::RSI;
    }
    return core::data::ExtensionType::UNKNOWN;
}

std::string core::data::ExtensionTypeTools::getExtensionTypeName(core::data::ExtensionType type) {
    switch (type) {
        case ExtensionType::RA:
            return "RA";
        case ExtensionType::RSI:
            return "RSI";
...
namespace core::data {
    ...
    id::TypeID DType::typeId{"DType", base::IDType::DType, "tp"};

    std::string DType::getValStr() const {
        return dTypeId.toString();
    }

    std::string DType::getTypeName() const {
        return DType::typeId.type_name;
    }

    base::RVM_ID &DType::getTypeID() const {
        return DType::typeId;
    }

    bool DType::updateData(const std::shared_ptr<RVM_Data> &newData) {
        // DType 应该不允许被修改
        return false;
    }

    bool DType::compare(const std::shared_ptr<RVM_Data> &other, const base::Relational &relational) const {
        if (relational == base::Relational::RE || relational == base::Relational::RNE) {
            const auto &res = other->getTypeID() == DType::typeId && this->dTypeId.fullEqualWith(static_pointer_cast<DType>(other)->dTypeId);
            return relational == base::Relational::RE == res;
        } else {
            throw base::errors::ArgumentError(unknown_, unknown_, unknown_, {});
        }
    }

    bool DType::convertToBool() const {
        return true;
    }

    std::shared_ptr<base::RVM_Data> DType::copy_ptr() const {
        return std::make_shared<DType>(this->dTypeId);
    }

    id::TypeID DType::getDTypeID() const {
        return dTypeId;
    }

    id::TypeID Null::typeId{"Null", base::IDType::Null, "tp-null"};
    ...
    id::TypeID Numeric::typeId{"Numeric", base::IDType::Numeric, undefined_};
    ...
    id::TypeID Int::typeId{"Int", std::make_shared<id::TypeID>(Numeric::typeId), base::IDType::Int, tp_int};
    ...
    std::shared_ptr<Numeric> Int::executeOperator(const std::shared_ptr<Numeric> &other, Op op, const std::string &opstr) const {
        ...
        switch (other->getTypeID().idType) {
            case base::IDType::Int: {
                if (const auto otherInt = static_pointer_cast<const Int>(other)) {
                    return std::make_shared<Int>(op(value, otherInt->value));
                }
                break;
            }
            case base::IDType::Float: {
                if (const auto otherFloat = static_pointer_cast<const Float>(other)) {
                    return std::make_shared<Float>(op(static_cast<double>(value), otherFloat->getValue()));
                }
                break;
            }
            case base::IDType::Char: {
                if (const auto otherChar = static_pointer_cast<const Char>(other)) {
                    return std::make_shared<Int>(op(value, otherChar->getValue()));
                }
                break;
            }
            case base::IDType::Bool: {
                if (const auto otherBool = static_pointer_cast<const Bool>(other)) {
    ...
    bool Int::updateData(const std::shared_ptr<RVM_Data>& newData) {
        if (const auto& typeId_ = newData->getTypeID();
            typeId_.fullEqualWith(typeId)) {
            value = static_pointer_cast<const Int>(newData)->getValue();
            return true;
        }
        return false;
    }
    ...
    bool Int::compare(const std::shared_ptr<RVM_Data> &other, const base::Relational &relational) const {
        ...
        switch (relational) {
            case base::Relational::RG: return value > otherValue;
            case base::Relational::RGE: return value >= otherValue;
    ...
    std::shared_ptr<Numeric> Int::pow(const std::shared_ptr<Numeric> &other) const {
        if (other->getTypeID() == typeId || other->getTypeID() == Char::typeId || other->getTypeID() == Bool::typeId){
    ...
    std::shared_ptr<Numeric> Int::mod(const std::shared_ptr<Numeric> &other) const {
            auto moduloOp = []<typename T0, typename T1>(T0 a, T1 b) {
            if (b == 0) throw std::runtime_error("Division by zero in modulo operation");
        ...
        return executeOperator(other, moduloOp, "%");
    }
    ...
    id::TypeID Float::typeId{"Float", std::make_shared<id::TypeID>(Numeric::typeId), base::IDType::Float, tp_float};
    ...
    std::shared_ptr<Numeric> Float::executeOperator(const std::shared_ptr<Numeric> &other, Op op) const {
        ...
        if (other->getTypeID().fullEqualWith(Int::typeId)) {
            if (const auto otherInt = std::static_pointer_cast<const Int>(other)) {
                return std::make_shared<Float>(Float(op(value, otherInt->getValue())));
            }
        } else if (other->getTypeID().fullEqualWith( typeId)) {
            if (const auto otherFloat = std::static_pointer_cast<const Float>(other)) {
                return std::make_shared<Float>(Float(op(value, otherFloat->getValue())));
            }
        } else if (other->getTypeID().fullEqualWith( Char::typeId)) {
            if (const auto otherChar = std::static_pointer_cast<const Char>(other)) {
                return std::make_shared<Float>(Float(op(value, otherChar->getValue())));
            }
        } else if (other->getTypeID().fullEqualWith( Bool::typeId)) {
            if (const auto otherBool = std::static_pointer_cast<const Bool>(other)) {
    ...
    std::shared_ptr<Numeric> Float::divide(const std::shared_ptr<Numeric> &other) const {
        ...
        if (const auto &otherType = other->getTypeID();
            otherType.fullEqualWith(Int::typeId) || otherType.fullEqualWith(Char::typeId) ||
            otherType.fullEqualWith(Bool::typeId)) {
            if (const auto otherInt = std::static_pointer_cast<const Int>(other)) {
        ...
        } else if (otherType.fullEqualWith(typeId)) {
            if (const auto otherFloat = std::static_pointer_cast<const Float>(other)) {
                if (otherFloat->getValue() == 0.0) {
                    throw std::runtime_error("Division to zero.");
                }
                if (const double result = static_cast<double>(value) / otherFloat->getValue();
                    result == std::floor(result)) {
                    return std::make_shared<Int>(Int(static_cast<int>(result)));
                }
                else {
    ...
    std::shared_ptr<Numeric> Float::mod(const std::shared_ptr<Numeric> &other) const {
        auto moduloOp = []<typename T0, typename T1>(T0 a, T1 b) {
            if (b == 0) throw std::runtime_error("Division by zero in modulo operation");
            if constexpr (std::is_floating_point_v<T0> ||
                          std::is_floating_point_v<T1>) {
    ...
    bool Float::updateData(const std::shared_ptr<RVM_Data> &newData) {
        if (typeId.fullEqualWith(newData->getTypeID())) {
            if (const auto newFloat = std::static_pointer_cast<const Float>(newData)) {
    ...
    std::shared_ptr<Iterable> String::subpart(const int begin, const int end) {
    ...
    void String::splice(const std::shared_ptr<Iterable>& other)
    {
        if (!other->getTypeID().fullEqualWith(typeId))
        {
            throw base::errors::DataTypeMismatchError(unknown_, unknown_, {
                "Error data type: " + other->getTypeID().toString(),
                "Expected data type: " + other->getTypeID().toString()}, {
                "Data of 'tp-str' type can only be concatenated with data of 'tp-str' type."});
        }
        value += other->getValStr();
    }

    bool String::contains(const std::shared_ptr<RVM_Data>& data)
    {
        if (!data->getTypeID().fullEqualWith(Char::typeId))
        {
            throw base::errors::DataTypeMismatchError(unknown_, unknown_, {
                "Error data type: " + data->getTypeID().toString(),
                "Expected data type: " + data->getTypeID().toString()}, {
                "Data of 'tp-str' type can only be concatenated with data of 'tp-char' type."});
        }
        return value.find(static_pointer_cast<const Char>(data)->getValue()) != std::string::npos;
     }

    id::TypeID List::typeId("List", std::make_shared<id::TypeID>(Iterable::typeId), base::IDType::List, tp_list);
    ...
    std::shared_ptr<Iterable> List::subpart(const int begin, const int end) {
    ...
    std::string List::getValStdStr(const base::RVM_ID &main_container_id,
        ...
        for (size_t i = 0; i < dataList.size(); ++i) {
            ...
            if (std::ranges::find(main_container_ids,
    ...
    void List::splice(const std::shared_ptr<Iterable>& other)
    {
        if (!other->getTypeID().fullEqualWith(typeId))
        {
            throw base::errors::DataTypeMismatchError(unknown_, unknown_,
                {"Error data type: " + other->getTypeID().toString(),
                "Expected data type: " + typeId.toString()},
                {"Only data of type 'tp-list' can be spliced into data of type 'tp-list'."});
        }
        const auto &list = std::static_pointer_cast<List>(other);
        this->dataList.insert(this->dataList.end(), list->dataList.begin(), list->dataList.end());
    }

    bool List::contains(const std::shared_ptr<RVM_Data>& data)
    {
        return std::ranges::find(this->dataList, data) != this->dataList.end();
    }

    id::TypeID Dict::typeId{"Dict", std::make_shared<id::TypeID>(Iterable::typeId), base::IDType::Dict, tp_dict};
    ...
    void Dict::append(const std::shared_ptr<RVM_Data> &data) {
        if (!data->getTypeID().fullEqualWith(KeyValuePair::typeId)) {
            throw base::errors::DataTypeMismatchError(unknown_, unknown_,
                {"Error data type: " + data->getTypeID().toString(),
                "Expected data type: " + KeyValuePair::typeId.toString()},
                {"Only data of type 'tp-pair' can be inserted into data of type 'tp-dict'."});
        }
        const auto &kvPair = std::static_pointer_cast<KeyValuePair>(data);
        if (const std::string &key = kvPair->getKey()->toEscapedString();
            !this->dataDict.contains(key)) {
            this->dataDict[key] = kvPair;
    ...
    std::shared_ptr<Iterable> Dict::subpart(const int begin, const int end) {
        ...
        std::unordered_map<std::string, std::shared_ptr<KeyValuePair>> subDict;
        subDict.reserve(end - begin);
        for (int i = begin; i < end; ++i) {
            const std::string &key = this->keyList[i];
            subDict[key] = this->dataDict.at(key);
        }
        std::vector subList(this->keyList.begin() + begin, this->keyList.begin() + end);
    ...
    void Dict::setDataAt(int index, const std::shared_ptr<RVM_Data> &data) {
        ...
        dataDict[keyList[index]] = std::static_pointer_cast<KeyValuePair>(data);
    }
    ...
    std::string Dict::getValStr() const {
        ...
        for (int i = 0; i < keyList.size(); ++i) {
            const auto &data = dataDict.at(keyList[i]);
            str += data->toKVPairString();
            if (i != keyList.size() - 1) {
                str += ", ";
            }
        }
    ...
    bool Dict::updateData(const std::shared_ptr<RVM_Data> &newData) {
        if (newData->getTypeID().fullEqualWith(typeId)) {
            const auto &otherDict = static_pointer_cast<Dict>(newData);
    ...
    bool Dict::compare(const std::shared_ptr<RVM_Data> &other, const base::Relational &relational) const {
        if (other->getTypeID() != typeId) {
            ...
            if (relational == base::Relational::AND) {
                return convertToBool() && other->convertToBool();
            }
            if (relational == base::Relational::OR) {
                return convertToBool() || other->convertToBool();
            }
            throw base::errors::DataTypeMismatchError(unknown_, unknown_, {}, {});
        }
    ...
    Dict::Dict(std::unordered_map<std::string, std::shared_ptr<KeyValuePair>> dataDict,
    ...
    std::shared_ptr<base::RVM_Data> Dict::getDataAt(const std::string &key) {
        ...
        return dataDict[key]->getValue();
    }

    void Dict::setDataAt(const std::string& key, const std::shared_ptr<KeyValuePair> &data) {
        if (!dataDict.contains(key)) {
            dataDict[key] = data;
    ...
    void Dict::eraseDataAt(const std::string& key) {
        if (!dataDict.contains(key)) {
            throw base::errors::KeyNotFoundError(unknown_, unknown_, {}, {});
        }
        dataDict.erase(key);
        keyList.erase(std::ranges::find(keyList, key));
    }

    Dict::Dict(const std::shared_ptr<List>& list) {
        if (!list) {
    ...
    void Dict::insertDataAt(int index, const std::shared_ptr<RVM_Data> &data) {
        ...
        const auto &new_kvp = std::static_pointer_cast<KeyValuePair>(data);
        keyList.insert(keyList.begin() + index, new_kvp->getKey()->getValStr());
        dataDict.insert(std::make_pair(new_kvp->getKey()->getValStr(), new_kvp));
    }
    ...
    void Dict::splice(const std::shared_ptr<Iterable>& other)
    {
        if (!other->getTypeID().fullEqualWith(typeId))
        {
            throw base::errors::DataTypeMismatchError(unknown_, unknown_, {
            "Error data type: " + other->getTypeID().toString(),
            "Expected data type: " + typeId.toString()}, {
            "Only data of type 'tp-dict' can be spliced into data of type 'tp-dict'."});
        }
        for (const auto &otherDict = std::static_pointer_cast<Dict>(other);
            const auto &[k, v] : otherDict->dataDict)
        {
            setDataAt(k, v);
        }
    }

    bool Dict::contains(const std::shared_ptr<RVM_Data>& data)
    {
        if (data->getTypeID().fullEqualWith(String::typeId))
        {
            return dataDict.contains(std::static_pointer_cast<String>(data)->getValStr());
        }
        if (data->getTypeID().fullEqualWith(KeyValuePair::typeId))
        {
            const auto &kvp = std::static_pointer_cast<KeyValuePair>(data);
            if (const auto &it = getDataAt(kvp->getKey()->toEscapedString()))
            {
                return it->compare(kvp->getValue(), base::Relational::RE);
            }
        } else
        {
            for (const auto& v : dataDict | std::views::values)
            {
                if (v->getValue()->compare(data, base::Relational::RE))
                {
                    return true;
                }
            }
    ...
    std::shared_ptr<Iterable> Series::subpart(const int begin, const int end) {
    ...
    Series::Series(const std::shared_ptr<RVM_Data>& list) {
    ...
    id::TypeID Structure::typeId{"Structure", base::IDType::Structure, undefined_};
    ...
    id::TypeID KeyValuePair::typeId{"KeyValuePair", base::IDType::KeyValuePair, tp_pair};
    ...
    bool KeyValuePair::updateData(const std::shared_ptr<RVM_Data> &newData) {
        if (typeId == newData->getTypeID()){
            const auto newKVP = static_pointer_cast<const KeyValuePair>(newData);
            this->key = newKVP->key;
    ...
    bool KeyValuePair::compare(const std::shared_ptr<RVM_Data> &other, const base::Relational &relational) const {
        ...
        switch (relational){
            ...
            default: throw base::errors::DataTypeMismatchError(unknown_, unknown_, {}, {});
        }
    }
    ...
    std::string KeyValuePair::toEscapedString() const {
        return getValStr();
    }

    void KeyValuePair::setKey(const std::shared_ptr<RVM_Data>& key) {
        this->key = key;
    }

    void KeyValuePair::setValue(const std::shared_ptr<RVM_Data>& value) {
        this->value = value;
    }

    std::shared_ptr<base::RVM_Data> KeyValuePair::getKey() const {
        return key;
    }

    std::shared_ptr<base::RVM_Data> KeyValuePair::getValue() const {
        return value;
    }

    std::string KeyValuePair::toKVPairString() const {
        return (key->getInstID().fullEqualWith(instID) ?
        key->getTypeID().toString() : key->toEscapedString()) + ": " +
            (value->getInstID().fullEqualWith(instID) ?
                value->getInstID().toString() : value->toEscapedString());
    }

    id::TypeID CompareGroup::typeId{"CompareGroup", base::IDType::CompareGroup, undefined_};
    ...
    bool CompareGroup::compare(const base::Relational &relational) const
    {
    ...
    id::TypeID CustomType::typeId {"CustomType", base::IDType::CustomType, undefined_};
    ...
    bool CustomType::hasFieldItself(const std::string &fieldName) const
    {
        return tpFields.contains(fieldName) || instFields.contains(fieldName);
    }

    bool CustomType::hasField(const std::string &fieldName) const
    {
    ...
    std::shared_ptr<base::RVM_Data> CustomType::getTpField(const std::string &fieldName) {
        ...
        if (tpFields.contains(fieldName)) {
    ...
    void CustomType::setTpField(const std::string &fieldName, const std::shared_ptr<base::RVM_Data> &fieldData) {
        if (tpFields.contains(fieldName)) {
            ...
            bool has_this_method_field = std::ranges::find(methodFields, fieldName) != methodFields.end();
            if (fieldData->getTypeID() == Callable::typeId && has_this_method_field) {
                methodFields.insert(fieldName);
            } else if (has_this_method_field) {
                methodFields.erase(std::ranges::find(methodFields, fieldName));
            }
    ...
    void CustomType::addInstField(const std::string &fieldName, const std::shared_ptr<base::RVM_Data> &fieldData) {
        ...
        instFields[fieldName] = fieldData ? fieldData : std::make_shared<Null>();
        if (fieldData && fieldData->getTypeID() == Callable::typeId) {
            methodFields.insert(fieldName);
        }
    }
    ...
    bool CustomType::hasTpFieldItself(const std::string &fieldName) const
    {
        return tpFields.contains(fieldName);
    }

    bool CustomType::hasInstFieldItself(const std::string &fieldName) const
    {
        return instFields.contains(fieldName);
    }

    bool CustomType::hasTpField(const std::string &fieldName) const
    {
    ...
    void CustomType::travalTypes(const std::function<bool(std::shared_ptr<CustomType>)>& callback) const
    {
        for (auto cur_type = parentType; cur_type != nullptr; cur_type = cur_type->parentType) {
    ...
    id::TypeID CustomInst::typeId{"CustomInst", base::IDType::CustomTypeInst, undefined_};
    ...
    CustomInst::CustomInst(std::shared_ptr<CustomType> instType)
        ...
        for (auto cur_type = customType; cur_type != nullptr; cur_type = cur_type->parentType){
            ...
            for (const auto& [field_name, field_data] : cur_type->instFields) {
                type_fields.emplace(field_name, field_data->copy_ptr());
            }
        }
    }
    ...
    CustomInst::getField(const std::string &fieldName, const std::shared_ptr<CustomType> &specCustomType) {
        ...
        if (specCustomType) {
            ...
            if (instFields.contains(specCustomType->typeName)) {
                if (const auto &fieldData = specCustomType->getTpField(fieldName)) {
                    return fieldData;
                }
                if (instFields[specCustomType->typeName].contains(fieldName)) {
            ...
            while (cur_type) {
                ...
                if (instFields[cur_type->typeName].contains(fieldName)) {
    ...
    void CustomInst::setField(const std::string &fieldName, const std::shared_ptr<base::RVM_Data> &fieldData,
        ...
        if (specCustomType) {
            ...
            } else if (instFields.contains(specCustomType->typeName)) {
    ...
    bool CustomInst::hasField(const std::string &fieldName) const
    {
    ...
    id::TypeID Callable::typeId{"Callable", base::IDType::Callable, tp_func};
    ...
    id::TypeID Function::typeId{"Function", std::make_shared<id::TypeID>(data::Callable::typeId), base::IDType::Function, tp_func};

    id::TypeID RetFunction::typeId{"RetFunction", std::make_shared<id::TypeID>(Function::typeId), base::IDType::RetFunction, tp_func};

    id::TypeID Quote::typeId ("Quote", base::IDType::Quote, tp_qot);

    Quote::Quote(const id::DataID &quoteDataID)
            : dataId(id::DataID()), quotedDataIDString(quoteDataID.toString()),
    ...
    void Quote::updateQuoteData(const std::shared_ptr<RVM_Data> &newData) const
    {
        memory::data_space_pool.updateDataNoLock(quoteDataID, newData);
    }

    std::shared_ptr<base::RVM_Data> Quote::getQuotedData() const {
        return memory::RVM_Memory::getInstance().findDataByIDNoLock(quoteDataID);
    }
    ...
    id::TypeID File::typeId {"File", base::IDType::File, tp_file};

    File::File(const FileMode &mode, const std::string &filepath) : dataId(id::DataID()),
                                                                    filepath(filepath), fileMode(mode){}
    ...
    void File::setModeByString(const std::string &modeStr) {
        if (!fileModeMap.contains(modeStr)){
    ...
    id::TypeID Time::typeId{"Time", base::IDType::Time, tp_time};
        ...
        if (other->getTypeID().fullEqualWith(typeId)){
            ...
            switch (relational) {
                ...
                case base::Relational::RL:
                    return thisTimeTuple < otherTimeTuple;
                case base::Relational::RG:
                    return thisTimeTuple > otherTimeTuple;
                case base::Relational::RLE:
                    return thisTimeTuple <= otherTimeTuple;
                case base::Relational::RGE:
                    return thisTimeTuple >= otherTimeTuple;
                case base::Relational::AND:
    ...
    id::TypeID Error::typeId{"Error", base::IDType::Error, tp_error};

    Error::Error(std::string error_position, std::string error_line, std::vector<std::string> error_info)
            : base::RVM_Data(), error_position(std::move(error_position)), error_line(std::move(error_line)),
              error_info(std::move(error_info)){
        error = std::make_shared<base::errors::ExposedError>(this->error_position, this->error_line, this->error_info);
    }

    std::string Error::getValStr() const {
    ...
    base::RVM_ID &Error::getTypeID() const {
        return typeId;
    }

    bool Error::updateData(const std::shared_ptr<RVM_Data> &newData) {
        if (!newData->getTypeID().fullEqualWith(typeId)) {
            return false;
        } else {
            const auto &new_error_data = std::static_pointer_cast<Error>(newData);
            error_line = new_error_data->error_line;
            error_info = new_error_data->error_info;
            error_position = new_error_data->error_position;
            error = new_error_data->error;
            return true;
        }
    }

    bool Error::compare(const std::shared_ptr<RVM_Data> &other, const base::Relational &relational) const {
        return false;
    }

    bool Error::convertToBool() const {
        return false;
    }

    std::shared_ptr<base::RVM_Data> Error::copy_ptr() const {
        return std::make_shared<Error>(error_position, error_line, error_info);
    }

    std::string Error::toString() const {
        return "[Error: " + dataId.toString() + "]";
    }

    id::TypeID Extension::typeId {"Extension", base::IDType::Extension, undefined_};

    Extension::Extension(const std::string &ident, const std::string &extension_path)
    : extension_type(ExtensionTypeTools::getExtensionType(extension_path)), extension_name(utils::getFileFromPath(extension_path)),
      extension_ident(ident), extension_path(extension_path){
        extension_scope = memory::RVM_Memory::getInstance().acquireScopeNoLock(extension_ident, false);
    }

    std::string Extension::getValStr() const {
        return "[Extension(" + ExtensionTypeTools::getExtensionTypeName(extension_type) +
        "): " + dataId.base::RVM_ID::toString("Data(" + extension_name + ")") + "]";
    }
    ...
    base::RVM_ID &Extension::getTypeID() const {
        return typeId;
    }

    bool Extension::updateData(const std::shared_ptr<RVM_Data> &newData) {
        return false;
    }

    bool Extension::compare(const std::shared_ptr<RVM_Data> &other, const base::Relational &relational) const {
        return false;
    }

    bool Extension::convertToBool() const {
        return true;
    }

    std::shared_ptr<base::RVM_Data> Extension::copy_ptr() const {
        return std::make_shared<Extension>(this->extension_name, this->extension_path);
    }

    Extension::Extension(const std::string &ident, const std::string &extension_path, const ScopePtr &scope)
    : extension_type(ExtensionTypeTools::getExtensionType(extension_path)),
      extension_name(utils::getFileNameFromPath(extension_path)),
      extension_ident(ident), extension_path(extension_path),
      extension_scope(scope){}

    Extension::Extension(const std::string &ident, const std::string &extension_name, const std::string &extension_path)
    : extension_type(ExtensionTypeTools::getExtensionType(extension_path)), extension_name(extension_name),
      extension_ident(ident), extension_path(extension_path){
        extension_scope = memory::RVM_Memory::getInstance().acquireScopeNoLock(extension_ident, false);
    }
...
namespace core::memory{
    std::unordered_map<std::string, std::shared_ptr<core::data::Extension>> loadedExtensions {};
    ...
    void RVM_Scope::addDataByName(const std::string &name, const std::shared_ptr<base::RVM_Data> &data) {
        checkOpPermission(base::OpMode::Add);
    ...
    bool RVM_Scope::updateDataByID(const id::DataID &id, const std::shared_ptr<base::RVM_Data>& newData) {
        checkOpPermission(base::OpMode::Update);
    ...
    bool RVM_Scope::removeDataByID(const id::DataID &dataId) {
        checkOpPermission(base::OpMode::Remove);
    ...
    void RVM_Scope::printInfo() const {
        ...
        out << "Addable: " << (addable ? "Y" : "N") << ", " <<
        "Deletable: " << (deletable ? "Y" : "N") << ", "
        << "Updatable: " << (updatable ? "Y" : "N") << "\n";
        out << "Findable: " << (findable ? "Y" : "N") << ", ";
        out << "Removable: " << (removable ? "Y" : "N") << "\n";
    ...
    bool RVM_Scope::isModifiable() const {
        return addable && updatable && deletable && findable;
    }

    void RVM_Scope::setModification(const std::tuple<bool, bool, bool, bool> &modification) {
        std::tie(addable, updatable, deletable, findable) = modification;
    }

    void RVM_Scope::checkOpPermission(const base::OpMode &opMode) const {
        bool res{};
        switch (opMode) {
            case base::OpMode::Add:
                res = addable;
                break;
            case base::OpMode::Delete:
                res = deletable;
                break;
            case base::OpMode::Update:
                res = updatable;
                break;
            case base::OpMode::Find:
                res = findable;
                break;
            case base::OpMode::Remove:
                res = removable;
                break;
        }
        if (!res) {
            throw base::errors::ModificationError(unknown_, unknown_,
                                                  {"This error is caused by a modification of a non-modifiable scope.",
                                                   "Error operation: " + base::OpModeTools::getOpModeString(opMode)},
                                                  {"Check whether the scope is modifiable."});
        }
    }

    bool RVM_Scope::isAddable() const {
        return addable;
    }

    void RVM_Scope::setAddable(bool isAddable) {
        RVM_Scope::addable = isAddable;
    }

    bool RVM_Scope::isUpdatable() const {
        return updatable;
    }

    void RVM_Scope::setUpdatable(bool isUpdatable) {
        RVM_Scope::updatable = isUpdatable;
    }

    bool RVM_Scope::isDeletable() const {
        return deletable;
    }

    void RVM_Scope::setDeletable(bool isDeletable) {
        RVM_Scope::deletable = isDeletable;
    }

    bool RVM_Scope::isFindable() const {
        return findable;
    }

    void RVM_Scope::setFindable(bool isFindable) {
        RVM_Scope::findable = isFindable;
    }

    bool RVM_Scope::isRemovable() const {
        return removable;
    }

    void RVM_Scope::setRemovable(bool isRemovable) {
        RVM_Scope::removable = isRemovable;
    }

    bool RVM_Scope::getOpPermission(const base::OpMode &opMode) const {
        switch (opMode) {
            case base::OpMode::Add:
                return addable;
            case base::OpMode::Delete:
    ...
    RVM_Memory::ScopePtr
    RVM_Memory::findOperableScopeByDataName(const std::string &name, bool needContains, const base::OpMode &opMode) const {
        for (auto it = activeScopes_.rbegin(); it != activeScopes_.rend(); ++it) {
            if ((*it)->getOpPermission(opMode) && needContains == (*it)->contains(name)){
                return *it;
            }
        }
        return nullptr;
    ...
    RVM_Memory::DataPair RVM_Memory::addData(const std::string &name, const RVM_Memory::DataPtr &data, const std::string &scopeName) const
    {
        std::lock_guard lock(mutex_);
        ScopePtr scope = scopeName.empty() ? getCurrentScopeNoLock() : findScopeByNameNoLock(scopeName);
        if (scopeName.empty() && (!scope || scope && !scope->isModifiable())){
            scope = findOperableScopeByDataName(name, false, base::OpMode::Add);
        }
        if (!scope) {
    ...
    RVM_Memory::DataPair RVM_Memory::addGlobalData(const std::string &name, const RVM_Memory::DataPtr &data) const
    {
    ...
    void RVM_Memory::addGlobalDataBatch(const std::unordered_map<std::string, DataPtr> &datas) const
    {
        for (const auto &[name, data]: datas) {
            const auto &_ = addGlobalData(name, data);
        }
    }
    ...
    void RVM_Memory::updateDataByID(const id::DataID &dataId, RVM_Memory::DataPtr newData) const
    {
        std::lock_guard lock(mutex_);
        updateDataNoLock(dataId, std::move(newData));
    }

    void RVM_Memory::updateDataByIDNoLock(const id::DataID &dataId, RVM_Memory::DataPtr newData) const
    {
        updateDataNoLock(dataId, std::move(newData));
    }

    void RVM_Memory::updateDataByName(const std::string &name, const RVM_Memory::DataPtr &newData) const
    {
        std::lock_guard lock(mutex_);
        updateDataByNameNoLock(name, newData);
    }

    void RVM_Memory::removeDataByID(const id::DataID &dataId) const
    {
        std::lock_guard lock(mutex_);
    ...
    size_t RVM_Memory::getFreeScopeCount() const
    {
        std::lock_guard lock(mutex_);
    ...
    void RVM_Memory::printPoolInfo() {
        ...
        out << "\n--- Extension details ---\n";
        for (const auto& ext : loadedExtensions | std::views::values) {
            out << ext->toString() << "\n";
            out << "Path: " << ext->extension_path << "\n";
            ext->extension_scope->printInfo();
        }

        out << "\n--- Recent Access Cache ---\n";
        size_t validCache = 0;
        for (const auto& [fst, snd] : recentAccessCache_ | std::views::values) {
            if (snd.lock()) {
                validCache++;
            }
        }
    ...
    void RVM_Memory::updateDataNoLock(const id::DataID &dataId, const RVM_Memory::DataPtr &newData) const {
        ...
        if (currentScope_){
            if(currentScope_->isModifiable() &&
            currentScope_->updateDataByID(dataId, newData)){
                return;
            }
        }
        if (const auto scope = findOperableScopeByDataName(dataId.getName(), true, base::OpMode::Update)){
    ...
    void RVM_Memory::updateDataByNameNoLock(const std::string &name, DataPtr newData) const
    {
        if (!newData) {
            throw base::errors::MemoryError(unknown_, unknown_, {}, {});
        }
        ...
        if (currentScope_){
            if (currentScope_->isUpdatable() && currentScope_->contains(name)){
        ...
        auto scope = findOperableScopeByDataName(name, true, base::OpMode::Update);
        if (scope && scope->updateDataByName(name, std::move(newData))) {
            return;
        }
        throw base::errors::MemoryError(unknown_, unknown_,{},{});
    }

    void RVM_Memory::removeDataNoLock(const id::DataID &varId) const
    {
        if (currentScope_){
    ...
    void RVM_Memory::removeScopeNoLock(const RVM_Memory::ScopePtr &scope) {
        const auto &name = scope->getName();
        if (!name.empty()) {
            namedScopes_.erase(name);
        }
    ...
    void RVM_Memory::removeScope(const RVM_Memory::ScopePtr &space) {
        if (!space) return;
        std::lock_guard<std::mutex> lock(mutex_);
        removeScopeNoLock(space);
    }

    void RVM_Memory::removeScope() {
        std::lock_guard<std::mutex> lock(mutex_);
        removeScopeNoLock(getCurrentScopeNoLock());
    }

    void RVM_Memory::appendScope(const RVM_Memory::ScopePtr &space) {
        if (!space) return;
        std::lock_guard<std::mutex> lock(mutex_);
        appendScopeNoLock(space);
    }

    void RVM_Memory::appendScopeNoLock(const RVM_Memory::ScopePtr &space) {
        if (!space || checkScopeExistNoLock(space)) return;
        activeScopes_.push_back(space);
        currentScope_ = space;

        if (!space->getName().empty()) {
            namedScopes_[space->getName()] = space;
        } else {
            const std::string &spaceName = space->getName().empty() ?
                                           DEFAULT_SCOPE_PREFIX + space->getInstID().toString()
                                           + std::to_string(nextDefaultNameId_++) :
                                           space->getName();
            space->setName(spaceName);
        }
    }

    bool RVM_Memory::checkScopeExist(const std::string &scopeName) const
    {
        if (scopeName.empty()) return false;
        return checkScopeExistNoLock(scopeName);
    }

    bool RVM_Memory::checkScopeExist(const RVM_Memory::ScopePtr &space) const
    {
        if (!space) return false;
        return checkScopeExistNoLock(space);
    }

    bool RVM_Memory::checkScopeExistNoLock(const std::string &scopeName) const
    {
        if (scopeName.empty()) return false;
        return namedScopes_.contains(scopeName);
    }

    bool RVM_Memory::checkScopeExistNoLock(const RVM_Memory::ScopePtr &space) const
    {
        if (!space) return false;
        return namedScopes_.contains(space->getName());
    }
}