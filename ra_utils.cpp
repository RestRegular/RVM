...
namespace utils {
    ...
    int stringToInt(const std::string &str) {
        if (str.empty()) {
            throw base::errors::TypeMismatchError(unknown_, str,{},{});
        }
        size_t i = 0;
    ...
    Number stringToNumber(const std::string &str) {
        ...
        } catch (const base::errors::TypeMismatchError &_) {
            try {
    ...
    // Pos具体实现
    Pos::Pos(int line, int column, std::string filepath) : line(line), column(column), filepath(std::move(filepath)) {}
    ...
    std::string Pos::toString() const {
        return "\"" + filepath + ":" + std::to_string(line) + ":" + std::to_string(column) + "\", line " +
