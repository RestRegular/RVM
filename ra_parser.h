...
namespace parser {
    ...
    namespace env {
        ...
        extern std::unordered_set<std::string> linked_extensions;
    ...
    namespace parse {
        ...
        std::shared_ptr<InsSet> parseCode(const std::string &type, std::string code_path,
                                          ...
                                          const std::optional<utils::Pos> &fixed_pos = std::nullopt);

        std::shared_ptr<InsSet> parseCodeFromPath(const std::string &code_file_path, bool output_info = false);
        ...
        std::shared_ptr<InsSet> getLinkedInsSet(const Ins &link_ins, bool check_exist = true);
    }
