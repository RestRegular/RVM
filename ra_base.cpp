...
namespace base {
    ...
    void RVMSerialHeader::validate(const std::string &file_path) const {
        const uint8_t expected_magic[] = {'R', 'S', 'I', 0x1A};
        if (memcmp(magic, expected_magic, sizeof(expected_magic)) != 0) {  // 固定使用 expected_magic 的长度
            throw std::runtime_error("Invalid file format!");
        }
        if (major_ver > VERSION_MAJOR || mid_ver > VERSION_MID || minor_ver > VERSION_MINOR) {
    ...
    std::string RVMSerialHeader::getRVMVersionInfo() const {
        return "RVM version " + std::to_string(VERSION_MAJOR) + "." + std::to_string(VERSION_MID) + "."
               + std::to_string(VERSION_MINOR) + " (on " + std::string(__DATE__) + " at " +
               std::string(__TIME__) + ")";
    }
    ...
    std::string RVMSerialHeader::getRSIVersionInfo(const std::string &path) const {
        ...
        return "[RSI File: '" + file_name + "'] version " + std::to_string(major_ver) + +"." +
               std::to_string(mid_ver) + "."
               + std::to_string(minor_ver) + " (" + getSerializationProfileName(getProfile()) + ")";
    }
    ...
    Relational stringToRelational(const std::string &str) {
        if (auto rel = relationalMap.find(str); rel != relationalMap.end()) {
            ...
            throw RVM_Error(ErrorType::ValueError, unknown_, unknown_,
                            ...
                            {"The relation string contains only 'RL', 'RLE', 'RNE', 'RE', 'RSE', 'RS', 'RT', 'RF'."});
        }
    }
        ...
        switch (relational) {
            ...
            default:
                throw RVM_Error(ErrorType::ValueError, unknown_, unknown_,
                                ...
                                {"The relation string contains only 'RL', 'RLE', 'RNE', 'RE', 'RSE', 'RS', 'RT', 'RF', 'AND', 'OR'."});
        }
    }
    ...
    std::string RVM_Data::getDataInfo() {
    ...
    std::string RVM_Error::toString() const {
        ...
        if (!error_position.empty() && error_position != unknown_) {
            oss << space << "[ Line ] : "
                << utils::StringManager::wrapText(error_position, 80, space_size + 11, "", "~ ") << "\n"
        ...
        if (!repair_tips.empty()) {
            ...
            for (size_t i = 0; i < repair_tips.size(); ++i) {
                oss << utils::StringManager::wrapText(repair_tips[i], 80, space_size + 11);
    ...
    std::string RVM_Error::getErrorInfo() const {
        ...
        for (size_t i = 0; i < error_info.size(); ++i) {
            oss << utils::StringManager::wrapText(error_info[i], 80, space.size() + 11);
    ...
    bool containsKeyword(const std::string &keyword) {
        for (const auto &category : base::categorizedKeywords) {
            const auto &keywords = category.second;
    ...
    bool containsKeywordInCategory(const KeywordType &category, const std::string &keyword) {
        ...
        // 获取分类的关键字集合
        const auto &keywords = base::categorizedKeywords[categoryName];
